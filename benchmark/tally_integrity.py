...
def _check_rows_list(checks) -> list[dict]:
    ...
    raise ``KeyError`` (#727). Mirrors the sanitizer the other gates use (e.g. ``row_integrity``).
    """
    if checks is None:
        return []
    ...
    for idx, row in enumerate(checks):
        ...
        if missing:
            logger.warning(
                "tally_integrity: checks[%s] missing required key(s) %s; skipping", idx, missing)
            continue
        name = row["name"]
        if not isinstance(name, str):
            logger.warning(
                "tally_integrity: checks[%s] name is %s, not str; skipping",
                idx, type(name).__name__)
            continue
        if not name.strip():
            logger.warning(
                "tally_integrity: checks[%s] name is blank; skipping", idx)
            continue
        if not isinstance(row["passed"], bool):
            logger.warning(
                "tally_integrity: checks[%s] passed is %s, not bool; skipping",
