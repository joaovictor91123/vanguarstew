"""CLI: print average tasks per scored repo from a replay artifact.

  python -m scripts.repo_task_mean result.json
"""
...
from benchmark.repo_task_mean import repo_task_mean_headline, summarize_repo_task_mean
...
def run(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Report average tasks per scored repo")
    ...
    try:
        ...
        return int(exc.code)
    summary = summarize_repo_task_mean(artifact)
    print(repo_task_mean_headline(summary), file=sys.stderr)
