"""CLI: print generalization gap outlook from a replay artifact.

  python -m scripts.gap_outlook result.json
"""
...
from benchmark.gap_outlook import gap_outlook_headline, summarize_gap_outlook
...
def run(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Report generalization gap outlook")
    ap.add_argument("artifact", help="run_eval --generalization --out JSON artifact")
    args = ap.parse_args(argv)
    try:
        artifact = load_artifact(args.artifact)
    except SystemExit as exc:
        return int(exc.code)
    summary = summarize_gap_outlook(artifact)
    print(gap_outlook_headline(summary), file=sys.stderr)
